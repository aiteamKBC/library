...
export default function CirculationHistoryManager() {
  ...
  const [expandedStudentCards, setExpandedStudentCards] = useState<Record<string, boolean>>({});
      ...
      {tab === "students" && (
        <div className="grid gap-4 sm:[grid-template-columns:repeat(auto-fit,minmax(420px,1fr))]">
          {filteredStudents.map((student) => {
            ...
            const visibleTitles = getVisibleStudentTitles(student.titles, sortBy);
            const isStudentExpanded = Boolean(expandedStudentCards[student.key]);
            const hasHiddenTitles = visibleTitles.length > 2;
            const titlesToRender = isStudentExpanded ? visibleTitles : visibleTitles.slice(0, 2);
                  ...
                  <div className="rounded-2xl border border-[#E9D9BD] bg-[#FCFAF6] px-4 py-4">
                    ...
                    {hasHiddenTitles && (
                      <button
                        type="button"
                        onClick={() =>
                          setExpandedStudentCards((previous) => ({
                            ...previous,
                            [student.key]: !previous[student.key],
                          }))
                        }
                        className="mt-3 inline-flex items-center gap-2 rounded-full border border-[#E9D9BD] bg-white px-4 py-2 text-xs font-semibold text-[#442F73] transition-colors hover:border-[#442F73]/25 hover:text-[#241453]"
                      >
                        {isStudentExpanded ? "Show fewer books" : `Show all ${visibleTitles.length} books`}
                        <i className={isStudentExpanded ? "ri-arrow-up-s-line" : "ri-arrow-down-s-line"} />
                      </button>
                    )}
                  </div>
          ...
          {filteredBooks.map((book) => (
            <div key={book.key} className="rounded-[28px] border border-gray-200 bg-gradient-to-br from-white via-white to-[#FCFAF6] p-5 shadow-sm">
              <div className="flex items-center justify-between gap-4">
                ...
                <div className="min-w-[120px] rounded-2xl border border-[#E9D9BD] bg-white px-5 py-4 text-center shadow-sm">
                  <p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-[#9B8AB8]">Returned</p>
                  <p className="mt-2 text-4xl font-bold leading-none text-[#241453]">{book.totalReturns}</p>
                </div>
              </div>
